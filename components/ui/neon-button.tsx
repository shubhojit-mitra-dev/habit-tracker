...
const buttonVariants = cva(
    "relative group border text-foreground mx-auto text-center rounded-full",
    {
        variants: {
            variant: {
                default: "bg-blue-500/20 hover:bg-blue-500/0 border-blue-500/20 transition-colors duration-300 cursor-pointer ease-in-out",
    ...
    ({ className, neon = true, size, variant, children, ...props }, ref) => {
        return (
            <button
                ...
                {...props}
            >
                <span className={cn("absolute h-px opacity-10 group-hover:opacity-100 transition-all duration-500 ease-in-out inset-x-0 inset-y-0 bg-gradient-to-r w-3/4 mx-auto from-transparent dark:via-blue-500 via-blue-600 to-transparent hidden", neon && "block")} />
                {children}
